## Workflow
1. Check if something already exists before determining what needs to be built
2. Identify missing context, ask the user, search the web, or check `node_modules` if you're unsure about something
3. Check surrounding and relevant files and adjust your coding style to match, do not introduce sloppy code
4. Write the code, optimizing for maintainability and readability with senior engineer standards
5. Verify by running `pnpm check`, then once all relevant errors are fixed, run `pnpm format` before declaring the task complete, do not undo the resulting formatted changes
6. Provide a brief summary of what was changed and any breaking decisions that had to be made
